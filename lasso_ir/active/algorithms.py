...
class BaseAlgorithm1:
    ...
    def select_features(self):
        self.mask = np.random.rand(1,X.shape[1])
        return np.vstack((X,self.mask))
...
class NearestNeighbor1(BaseAlgorithm1):
    """
        For each query, picks a random positive example seen so far and finds the closest unlabeled example to that.
        The base NN algorithm uses all features, but LassoNN reimplements "select_features"
        """
    ...
    def get_query(self):
        ...
        x = _X[-1]#_X[target]
        _X = _X[:-1]#_X[self.unlabeled]
        unlabeled = list(self.unlabeled)
        #print(len(unlabeled))
        _X = _X[unlabeled]
        #print(_X.shape)
        
        dists = np.matmul(_X, x)#np.linalg.norm(_X - x, axis=1)
        
        best = np.argmax(dists)#np.argmin(dists)
        #print(best)
        self.elapsed = time.time() - t
        return self.unlabeled[best]
...
class MargionalNN(NearestNeighbor):
    ...
    def select_features(self):
        ...
        if can_fit(_y):
            ...
            return np.vstack((X,self.mask))#X[:, self.mask!=0] * np.tile(self.mask[self.mask!=0].transpose(), (X.shape[0],1))
        else:
            self.mask = np.random.rand(1,X.shape[1])
            return np.vstack((X,self.mask))
...
class MargionalNNPlus(MargionalNN):
    ...
    def select_features(self):
        ...
        if can_fit(_y):
            ...
            self.mask = np.random.rand(X.shape[1])
            return np.vstack((X,self.mask))
...
class LassoNN(NearestNeighbor):
    ...
    def select_features(self):
        ...
        if can_fit(_y):
            ...
            mask = model.coef_#np.ravel(model.coef_.astype(bool))
            return np.vstack((X,mask)) #X[:, mask]
        else:
            self.mask = np.random.rand(X.shape[1])
            return np.vstack((X,self.mask))
...
class MargionalNN1(NearestNeighbor1):
    """
        Nearest Neighbor but uses Marginal Regression to select the features.
        Will select n_trained/N features, (N=1 by default)
        """
...
class MargionalNNPlus1(MargionalNN1):
    @property
    def name(self):
        return f"nn-marginalplus1-{self.N}"
...
class LassoNN1(NearestNeighbor1):
    """
        Nearest Neighbor but uses LASSO to select the features
        """
    name = "nn-lasso1"
